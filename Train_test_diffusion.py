...
def fem_residual_curve(pred_grids, dt, K=1.0):
    """Numeric PDE residual of the pure-FEM field on the point grid.

    pred_grids: list of [H, W] arrays (times dt, 2*dt, ...). Returns the same
    report dict shape as compute_residual_curve so the posterior path is
    ...
    d2u_dx2 = np.gradient(du_dx, axis=2) / (1.0 / (W - 1))
    d2u_dy2 = np.gradient(du_dy, axis=1) / (1.0 / (H - 1))
    u_t = np.gradient(u, axis=0) / dt if T > 1 else np.zeros_like(u)
    res = u_t - K * (d2u_dx2 + d2u_dy2)
    res2 = res ** 2
...
def report_from_predictions(trainer, args, pred_states, uncorrected_sol,
                            pred_grids, dt):
    if args.mode == "fem":
        return fem_residual_curve(pred_grids, dt, K=1.0)
...
if __name__ == "__main__":
    ...
    if args.exp_name is not None:
        ...
        exp_dir = make_exp_dir(args.base_output_dir, "diffusion", params)
